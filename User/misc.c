...
{
	uint16_t crc = 0xffff;
	uint8_t *crcbuf = (uint8_t *)buf;
	while (bsize--)
	{
		crc = (uint16_t)(crc >> 8) ^ RS485_CRCTbl[(crc ^ *crcbuf++) & 0xff];
	}    
	return crc;
}


const unsigned int CRC8_TAB[256] = {                
...
{
    unsigned char ucIndex;             //CRC8??????
    unsigned char ucCRC8 = 0;          //CRC8?????
                                       //??CRC8???
    while(ucLen--)
    {
